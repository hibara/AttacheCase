...
resourcestring
	...
	MSG_ERROR_NOT_BLOWFISH_ENCRYPTION      = 'バージョンがちがいます。復号できません。'+#13+
	                                         'ファイルはver.1のアタッシェケースで暗号化されていません。';
	MSG_ERROR_PASSWORD_WRONG               = 'パスワードがちがいます。復号できません。'+#13+
	                                         '場合によってはファイルが壊れている可能性もあります。';
	MSG_ERROR_OUT_FILE_OPEN                = '保存する先のファイルが開けません。不正なファイルパスか、他のアプリケーションが使用中の可能性があります。';
  ...
  //ラベル
	...
	LABEL_STATUS_DETAIL_COMPARE_FAILED     = 'コンペアで問題を見つけたようです。';
