======================================================================
アタッシェケース    ver.2.8.0.8β     2012/11/10
...
ver.2.8.0.8β  2012/11/10
              ・前バージョンの関連バグで、比較的大きいデータを複数ファ
                ...
                ※カレントバージョン（ver.2.75）では問題なく復号できます。
                  ...
                  には問題はなく、復号だけに問題があったということにな
                  ります。
              ・ノートパソコンなど小さな画面ではややウィンドウが大きい
                気がしたので、サイドメニュー含めてデザインのダウンサイ
                ジングを行った。
              ・デバッグ＆テストのとき不便だったので(笑)、ウィンドウの
                最小化・最大化などをコマンドラインオプションから指定で
                きるようにした（※ /ws=[0,1,2]）。
              【ソースレベルの変更点】
              ・主に TAttacheCaseFileDecrypt2.cpp の修正。zlib出力バッ
                ファを正常に扱っていない部分があり、ファイルの大きさに
                よっては（これは本当に大小関係なく起きます）、ファイル
                を正常に戻せないなどのバグが発生していた。
